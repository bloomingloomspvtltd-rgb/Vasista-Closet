        ...
        <div className="admin-card admin-section">
          ...
          {loading ? (
            ...
            <div className="admin-table">
              ...
              {orders.map((order) => (
                <div className="admin-table-row cols-5" key={order.id}>
                  <div>#{String(order.id || "").slice(-6)}</div>
                  <div className="admin-pill admin-pill-pending">
                    {order.status || "pending"}
                    {order.payment_status ? ` / ${order.payment_status}` : ""}
                    {order.payment_failure_reason ? ` (${order.payment_failure_reason})` : ""}
                  </div>
                  <div>{order.items ? order.items.length : 0}</div>
                  <div>{formatCurrency(order.total)}</div>
                  <div className="admin-table-actions">
                    {Array.isArray(order.refunds) && order.refunds.length > 0 ? (
                      <span className="admin-muted">{order.refunds.length} refund(s)</span>
                    ) : null}
